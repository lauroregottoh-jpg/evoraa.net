    ...
    <section className="relative z-10 overflow-hidden rounded-[1.85rem] border-2 border-[#B8954A]/55 bg-gradient-to-br from-[#120f10] via-[#2A1810] to-[#5C1F28] text-[#F8F4EE] px-4 py-10 sm:px-8 sm:py-12 shadow-elevated min-h-[28rem] sm:min-h-[32rem]">
      ...
      <div className="relative z-10 mx-auto h-56 sm:h-64 w-full max-w-lg">
        ...
        {ITEMS.map((item, i) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                "absolute left-1/2 top-1/2 z-30 flex items-center gap-2 rounded-xl border border-[#B8954A]/45 bg-[#1C1412]/95 px-3 py-2 shadow-lg backdrop-blur-sm transition-all duration-700",
                show
        ...
        >
          ...
          {/* Corps */}
          <div className="relative mx-auto -mt-1 flex h-28 w-44 sm:h-32 sm:w-52 items-center justify-center rounded-b-2xl border-2 border-[#B8954A]/80 bg-gradient-to-b from-[#C9A227] to-[#7A5F28] shadow-[0_20px_50px_-12px_rgba(184,149,74,0.85)]">
            <div className="absolute inset-x-3 top-3 h-1 rounded-full bg-[#F3D9A4]/35" />
            <Gift
              className={cn(
                "h-12 w-12 text-[#1C1412] transition-all duration-500",
                phase === "burst" && "scale-90 opacity-50"
              )}
            />
            <div className="absolute inset-x-6 bottom-4 h-2 rounded-full bg-[#1C1412]/15" />
          </div>
        </div>
