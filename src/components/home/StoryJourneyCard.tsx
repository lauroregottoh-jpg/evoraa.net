...
export function StoryJourneyCard() {
  ...
  return (
    <div className="relative mx-auto w-full max-w-xl">
      ...
      />
      <div className="relative overflow-hidden rounded-3xl border border-white/15 bg-white/[0.08] p-5 shadow-elevated backdrop-blur-xl sm:p-7">
        <div className="flex items-center justify-between border-b border-white/10 pb-4">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-accent">
              Aperçu de votre parcours
            </p>
            <p className="mt-1 text-sm text-white/65">Une rencontre, étape après étape</p>
          </div>
        ...
        <div className="my-6 flex items-center justify-center">
          ...
          <div className="relative mx-3 h-px flex-1 bg-gradient-to-r from-white/10 via-accent to-white/10">
            <Heart className="absolute left-1/2 top-1/2 h-8 w-8 -translate-x-1/2 -translate-y-1/2 rounded-full bg-accent p-2 text-[#1C1412] shadow-lg" />
          </div>
          <ProfileInitial initials="PC" label="Profil compatible" accent />
        </div>

        <div className="relative min-h-40 overflow-hidden rounded-2xl border border-white/10 bg-black/20 p-5">
          <div key={active} className="animate-in fade-in slide-in-from-bottom-3 duration-500">
            ...
            <p className="mt-2 text-sm leading-relaxed text-white/70">{moment.detail}</p>
          </div>
        </div>
