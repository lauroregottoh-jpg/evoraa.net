...
export function CoachingWalletPanel() {
  ...
  return (
    <section className="rounded-2xl border border-[#2D1020]/15 bg-[#F2EBE0] p-5 space-y-4">
      <div>
      ...
      <form onSubmit={onLink} className="space-y-2">
        <label className="block text-sm font-medium text-[#2D1020]">
          Code coach
          <input
            value={code}
            onChange={(e) => setCode(e.target.value)}
            placeholder="ex. KE-4827"
            className="mt-1 w-full rounded-xl border border-[#2D1020]/20 bg-white px-3 py-2 text-sm"
        ...
        <button
          ...
          className="rounded-xl bg-[#2D1020] text-[#F2EBE0] px-4 py-2 text-sm font-semibold disabled:opacity-50"
        >
          Associer mon coach
        </button>
      </form>
      ...
      {msg ? (
        <p className="text-sm text-[#2D1020] bg-[#2D1020]/08 rounded-lg px-3 py-2">
          {msg}
        </p>
      ) : null}
      ...
      {coaches.length > 0 ? (
        ...
        <p className="text-xs text-[#2D1020]/55">
