...
export function CoachingSalesPage({
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 pb-10">
      <header className="relative overflow-hidden rounded-[1.75rem] border border-primary/15 bg-gradient-to-br from-[#7F5557] via-[#4A1820] to-[#3D2A14] px-5 py-8 sm:px-8 sm:py-10 text-[#F2EBE0] shadow-elevated">
        <div
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            {hasCredits ? (
              <Link
                ...
                className="inline-flex h-11 items-center rounded-xl bg-[#D4AF72] px-5 text-sm font-bold text-[#7F5557]"
              >
                Faire votre session
              </Link>
            ) : (
      ...
      <section className="relative overflow-hidden rounded-2xl border border-[#7F5557]/15 bg-[#F2EBE0] p-5 sm:p-6">
        <div className="flex items-start gap-3 mb-4">
          <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#7F5557]/10 text-[#7F5557]">
          ...
          <div>
            ...
            <h2 className="font-serif text-xl font-bold text-[#7F5557]">
              Votre salle audio privée
            </h2>
          </div>
        </div>
        <ul className="space-y-2.5 text-sm text-[#7F5557]/80">
          {[
            ...
            <li key={line} className="flex gap-2 items-start">
              {hasCredits ? (
                <CheckCircle2 className="h-4 w-4 text-[#7F5557] shrink-0 mt-0.5" />
              ) : (
                <Lock className="h-4 w-4 text-[#7F5557]/45 shrink-0 mt-0.5" />
              )}
              <span className={hasCredits ? "" : "opacity-80"}>{line}</span>
            </li>
          ))}
        </ul>
        <p className="mt-4 text-xs text-[#7F5557]/55 leading-relaxed">
          Un compte-rendu de séance est généré automatiquement pour le suivi.
          Les échanges restent dans KELIAA (messages prédéfinis).
        </p>
        <Link
          href="/coaching/session"
          className="mt-4 inline-flex text-sm font-semibold text-[#7F5557] underline underline-offset-4"
        >
          {hasCredits
