        ...
        switch (block.type) {
          case "h2":
            return (
              <h3
                key={key}
                className="font-serif text-[18pt] sm:text-[20pt] font-bold text-[#A07070] pt-2 leading-snug"
              >
                {block.text}
              </h3>
            )
          case "paragraph":
            return (
              <p
                key={key}
                className="text-[14pt] leading-[1.65] text-[#A07070]"
              >
                {block.text}
              </p>
            )
          ...
          case "callout": {
            const tone =
              block.tone === "alert"
                ? "border-[#A07070]/35 bg-[#A07070]/08"
                : block.tone === "info"
                  ? "border-[#A07070]/15 bg-[#F2EBE0]"
          ...
          case "fillBlank":
            return (
              ...
              >
                <p className="text-sm font-semibold text-[#A07070] mb-3">
                  {block.prompt}
                </p>
                <div
                  className="space-y-3"
                ...
                >
                  {Array.from({ length: block.lines ?? 2 }).map((_, li) => (
                    <div
                      key={li}
                      className="border-b border-[#A07070]/20 h-8"
                    />
                  ))}
                </div>
              </div>
          ...
          case "rolePlay":
            return (
              <div
                key={key}
                className="rounded-2xl border border-[#A07070]/20 bg-[#F2EBE0] p-5 space-y-3"
              >
                ...
                <p className="text-[14pt] leading-[1.65] text-[#A07070]">
                  {block.scene}
                </p>
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-xl bg-[#A07070]/08 p-4">
                    <p className="text-[12pt] font-bold uppercase tracking-wider text-[#A07070]">
                      Rôle A
                    </p>
          ...
          case "cycleFlow":
            return (
              <div
                key={key}
                className="rounded-2xl border border-[#A07070]/20 bg-[#F2EBE0] p-5 sm:p-6"
              >
                <p className="text-[12pt] font-bold uppercase tracking-[0.16em] text-[#B8954A]">
                  Schéma
                </p>
                <p className="font-serif text-[18pt] font-bold text-[#A07070] mt-1">
                  {block.title}
                </p>
                <ol className="mt-4 space-y-0">
                  {block.steps.map((step, si) => (
                    <li key={step} className="flex flex-col items-stretch">
                      <div className="rounded-xl border border-[#A07070]/15 bg-white px-4 py-3 text-[14pt] leading-[1.5]">
                      ...
                      {si < block.steps.length - 1 ? (
                        <div className="flex justify-center py-1 text-[#A07070] text-[18pt] leading-none">
                          ↓
                        </div>
                      ) : null}
                    </li>
          ...
          case "visualCards":
            return (
              <div key={key} className="space-y-3">
                <p className="font-serif text-[18pt] font-bold text-[#A07070]">
                  {block.title}
                </p>
                <div className="grid gap-3 sm:grid-cols-2">
