        ...
        switch (block.type) {
          case "h2":
            return (
              <h3
                key={key}
                className="font-serif text-[18pt] sm:text-[20pt] font-bold text-[#7F5557] pt-2 leading-snug"
              >
                {block.text}
              </h3>
            )
          case "paragraph":
            return (
              <p
                key={key}
                className="text-[14pt] leading-[1.65] text-[#7F5557]"
              >
                {block.text}
              </p>
            )
          ...
          case "callout": {
            const tone =
              block.tone === "alert"
                ? "border-[#7F5557]/35 bg-[#7F5557]/08"
                : block.tone === "info"
                  ? "border-[#7F5557]/15 bg-[#F2EBE0]"
          ...
          case "fillBlank":
            return (
              ...
              >
                <p className="text-sm font-semibold text-[#7F5557] mb-3">
                  {block.prompt}
                </p>
                <div
                  className="space-y-3"
                ...
                >
                  {Array.from({ length: block.lines ?? 2 }).map((_, li) => (
                    <div
                      key={li}
                      className="border-b border-[#7F5557]/20 h-8"
                    />
                  ))}
                </div>
              </div>
          ...
          case "rolePlay":
            return (
              <div
                key={key}
                className="rounded-2xl border border-[#7F5557]/20 bg-[#F2EBE0] p-5 space-y-3"
              >
                ...
                <p className="text-[14pt] leading-[1.65] text-[#7F5557]">
                  {block.scene}
                </p>
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-xl bg-[#7F5557]/08 p-4">
                    <p className="text-[12pt] font-bold uppercase tracking-wider text-[#7F5557]">
                      Rôle A
                    </p>
          ...
          case "cycleFlow":
            return (
              <div
                key={key}
                className="rounded-2xl border border-[#7F5557]/20 bg-[#F2EBE0] p-5 sm:p-6"
              >
                <p className="text-[12pt] font-bold uppercase tracking-[0.16em] text-[#B8954A]">
                  Schéma
                </p>
                <p className="font-serif text-[18pt] font-bold text-[#7F5557] mt-1">
                  {block.title}
                </p>
                <ol className="mt-4 space-y-0">
                  {block.steps.map((step, si) => (
                    <li key={step} className="flex flex-col items-stretch">
                      <div className="rounded-xl border border-[#7F5557]/15 bg-white px-4 py-3 text-[14pt] leading-[1.5]">
                      ...
                      {si < block.steps.length - 1 ? (
                        <div className="flex justify-center py-1 text-[#7F5557] text-[18pt] leading-none">
                          ↓
                        </div>
                      ) : null}
                    </li>
          ...
          case "visualCards":
            return (
              <div key={key} className="space-y-3">
                <p className="font-serif text-[18pt] font-bold text-[#7F5557]">
                  {block.title}
                </p>
                <div className="grid gap-3 sm:grid-cols-2">
