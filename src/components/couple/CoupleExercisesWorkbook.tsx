  ...
  return (
    <div className="mt-3 space-y-3">
      {Array.from({ length: lines }).map((_, i) => (
        <div key={i} className="h-9 border-b border-[#2D1020]/25" />
      ))}
    </div>
  )
}
...
function ExerciseCard({
  ...
  return (
    <article
      className={cn(
        "break-inside-avoid rounded-[1.75rem] border border-[#2D1020]/10 bg-white p-6 sm:p-9 shadow-sm space-y-5",
        ex.premiumPlus && "border-[#B8954A]/40"
      )}
    >
      <header className="space-y-2">
        {ex.premiumPlus ? (
          <span className="inline-block text-[10px] font-bold uppercase tracking-[0.18em] text-[#2D1020] bg-[#2D1020]/10 px-2.5 py-1 rounded-full">
            Premium Plus
          </span>
        ) : null}
        ...
        <p className="text-base sm:text-lg leading-relaxed text-[#2D1020]/85">
          <span className="font-semibold text-[#2D1020]">Objectif — </span>
          {ex.objective}
        </p>
        <p className="text-base leading-relaxed text-[#2D1020]/75">{ex.why}</p>
      ...
      <div>
        <h3 className="font-serif text-xl font-bold text-[#2D1020]">Consignes</h3>
      ...
      {ex.questions.length ? (
        <div>
          <h3 className="font-serif text-xl font-bold text-[#2D1020]">Questions</h3>
      ...
      {(ex.fillPrompts?.length ? ex.fillPrompts : ["Écrire ici…"]).map(
        (prompt) => (
          ...
          >
            <p className="text-base font-semibold text-[#2D1020]">{prompt}</p>
            <WriteLines lines={3} />
          </div>
        )
      )}
      ...
      {ex.rolePlay ? (
        <div className="rounded-2xl border border-[#2D1020]/20 bg-gradient-to-br from-[#F2EBE0] to-white p-5 sm:p-6 space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-xl bg-[#2D1020]/10 p-4">
              <p className="text-[10px] font-bold uppercase tracking-wider text-[#2D1020]">
                {nameA} — Rôle A
              </p>
      ...
      <div className="space-y-2 text-base leading-relaxed border-t border-[#2D1020]/10 pt-4">
        <p>
        ...
        <p className="italic text-[#2D1020]">
...
function buildExercisesHtml(doc: CoupleReportDocument): string {
  ...
  const parts = [
    ...
    `<style>
      body{font-family:Georgia,serif;max-width:740px;margin:2rem auto;padding:0 1.25rem;color:#2D1020;line-height:1.7;background:#F2EBE0;font-size:16px}
      h1{font-size:1.9rem} h2{font-size:1.45rem;color:#2D1020;margin-top:2.5rem;border-top:1px solid #ddd;padding-top:1.25rem}
  ...
  return (
    <div className={cn("max-w-3xl mx-auto space-y-8 pb-16", className)}>
      <header className="space-y-3">
        ...
        <p className="text-base sm:text-lg leading-relaxed text-[#2D1020]/75 max-w-xl">
        ...
        <div className="flex flex-wrap gap-2 print:hidden">
          <button
            ...
            className="inline-flex h-11 items-center gap-2 rounded-xl border border-[#2D1020]/15 bg-white px-4 text-sm font-semibold"
          >
          ...
          <button
            ...
            className="inline-flex h-11 items-center gap-2 rounded-xl bg-[#2D1020] px-4 text-sm font-semibold text-[#F2EBE0]"
          >
