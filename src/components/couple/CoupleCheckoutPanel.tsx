...
export function CoupleCheckoutPanel({
  ...
  return (
    <section
      id="payer"
      className={cn(
        "rounded-2xl border border-[#7F5557]/10 bg-white p-5 sm:p-7 space-y-6 shadow-sm",
        className
      )}
    >
      <div className="space-y-1">
        <p className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.18em] text-[#7F5557]">
          <HeartHandshake className="h-3.5 w-3.5" />
          Paiement sécurisé
        </p>
        <h2 className="font-serif text-2xl sm:text-3xl font-bold text-[#7F5557]">
      ...
      {demoPricing && !promoAmount && (
        <div className="rounded-xl border border-[#B8954A]/35 bg-[#B8954A]/10 px-4 py-3 text-sm">
          <p className="font-semibold text-[#7F5557]">
            Mode démo — {COUPLE_DEMO_AMOUNT_XOF} FCFA
          </p>
          <p className="text-xs text-[#7F5557]/65 mt-1 leading-relaxed">
          ...
          return (
            <button
              ...
              className={cn(
                ...
                active
                  ? "border-[#7F5557] bg-[#F2EBE0] shadow-sm"
                  : "border-[#7F5557]/12 hover:border-[#7F5557]/40 bg-white"
              )}
            >
              {o.popular && (
                <span className="absolute -top-2.5 left-4 rounded-full bg-[#B8954A] text-[#7F5557] text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5">
                  Le plus choisi
                </span>
              )}
              <div className="flex items-start justify-between gap-3 pt-1">
                <div className="flex items-start gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-sm mt-1 text-[#7F5557]/70 leading-snug">
                      {o.description}
                    </p>
                    <ul className="mt-2 space-y-0.5 text-xs text-[#7F5557]/65">
                ...
                <div className="shrink-0 text-right">
                  {(demoPricing || promoAmount) && amount !== o.amountXof && (
                    <p className="text-xs text-[#7F5557]/45 line-through">
                  ...
                  <p className="font-serif text-xl font-bold text-[#7F5557]">
                    {amount.toLocaleString("fr-FR")}
                    <span className="text-xs font-sans font-medium text-[#7F5557]/55 ml-1">
                      FCFA
                    </span>
                  </p>
                </div>
      ...
      <div className="rounded-xl border border-[#7F5557]/12 bg-[#F2EBE0] px-4 py-4 space-y-3">
        <p className="text-sm font-semibold text-[#7F5557] inline-flex items-center gap-2">
          <Tag className="h-4 w-4 text-[#7F5557]" />
          J&apos;ai un code
        </p>
        <div className="flex flex-wrap gap-2">
          <input
            ...
            autoComplete="off"
            className="flex-1 min-w-[12rem] rounded-xl border border-[#7F5557]/15 bg-white px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#B8954A]/50"
          />
          <button
            type="button"
            onClick={() => void applyPromo()}
            className="rounded-xl bg-[#7F5557] text-white px-4 py-2.5 text-sm font-semibold hover:brightness-110"
          >
            Appliquer
          </button>
        </div>
        {promoHint && (
          <p
            className={cn(
              "text-xs",
              promoAmount ? "text-[#7F5557] font-semibold" : "text-destructive"
            )}
          >
            {promoHint}
          </p>
      ...
      <div className="flex flex-wrap items-end justify-between gap-3 border-t border-[#7F5557]/10 pt-4">
        <div>
          <p className="text-xs text-[#7F5557]/55">Total à payer</p>
          <p className="font-serif text-2xl font-bold text-[#7F5557] flex items-baseline gap-2">
            {promoAmount ? (
              <span className="text-lg text-[#7F5557]/40 line-through font-sans font-normal">
                {baseCharge.toLocaleString("fr-FR")}
              </span>
            ) : null}
            {charge.toLocaleString("fr-FR")} FCFA
          </p>
          <p className="text-[11px] text-[#7F5557]/55">
      ...
      <button
        ...
        onClick={() => void pay()}
        disabled={loading}
        className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-[#7F5557] text-white h-12 px-6 text-sm font-bold hover:brightness-110 disabled:opacity-60"
      >
        {loading
      ...
      <p className="text-center text-[11px] text-[#7F5557]/55">
      ...
      <p className="text-center text-xs text-[#7F5557]/55">
        Déjà un code d&apos;invitation partenaire ?{" "}
        <Link href="/couple/rejoindre" className="font-semibold text-[#7F5557]">
          Rejoindre le bilan →
        </Link>
      </p>
    </section>
    ...
    <button
      ...
      className={cn(
        ...
        active
          ...
          : "border-[#7F5557]/12 hover:border-[#B8954A]/40"
      )}
    >
      <div className="flex items-center gap-2 text-[#7F5557] font-semibold text-sm">
        <span className="text-[#7F5557]">{icon}</span>
        {title}
      </div>
      <div className="mt-3 flex flex-wrap gap-1.5">
        {brands.map((b) => (
          <span
            key={b}
            className="rounded-md bg-white border border-[#7F5557]/10 px-2 py-0.5 text-[10px] font-semibold text-[#7F5557]/55"
          >
            {b}
          </span>
        ))}
