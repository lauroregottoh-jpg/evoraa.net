...
export function CouplePaywallOverlay({
  ...
  return (
    <div className={cn("relative", className)}>
      ...
      <div className="absolute inset-0 z-10 flex items-center justify-center p-4 bg-[#FCFAF6]/55 backdrop-blur-[2px]">
        <div className="w-full max-w-md rounded-2xl border border-[#D7B866]/40 bg-white shadow-elevated p-6 space-y-4 text-center">
          ...
          <h2 className="font-serif text-2xl font-bold text-[#641F2B]">{title}</h2>
          ...
          <form onSubmit={submitCode} className="space-y-2 text-left">
            ...
            <button
              ...
              className="w-full h-11 rounded-xl bg-[#641F2B] text-sm font-semibold text-[#FCFAF6] disabled:opacity-60"
            >
          ...
          <div className="flex flex-col sm:flex-row gap-2 justify-center pt-1">
            <Link
              href="/couple/offre"
              className="inline-flex h-11 items-center justify-center rounded-xl bg-[#641F2B] px-5 text-sm font-semibold text-[#FCFAF6]"
            >
              Faire le paiement — débloquer
            </Link>
            <Link
              href="/couple/rejoindre"
              className="inline-flex h-11 items-center justify-center rounded-xl border border-[#641F2B]/25 px-5 text-sm font-semibold text-[#641F2B]"
            >
...
export function CoupleLockedCard({
  ...
  return (
    <div className="relative rounded-2xl border border-[#2B2421]/10 bg-[#FFFDF9]/90 p-5 opacity-90">
      <div className="flex gap-3 items-start">
        <Lock className="h-5 w-5 text-[#641F2B]/50 shrink-0 mt-0.5" />
        <div className="min-w-0 flex-1">
          <p className="font-serif text-lg font-bold text-[#2B2421]/70">{title}</p>
          <p className="mt-1 text-sm text-[#2B2421]/55">{description}</p>
          <Link
            href={onUnlockHref}
            className="mt-3 inline-flex text-xs font-bold uppercase tracking-wider text-[#641F2B]"
          >
            Débloquer →
          </Link>
        </div>
