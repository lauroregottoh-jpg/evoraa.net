...
export function CoupleUnlockReveal({
  ...
  return (
    <section className="relative overflow-hidden rounded-[1.85rem] border-2 border-[#B8954A]/40 bg-gradient-to-br from-[#2D1020] via-[#3D1830] to-[#2D1020] text-[#F2EBE0] px-5 py-10 sm:px-8 sm:py-12 shadow-lg">
      {/* Confettis */}
      {phase !== "closed" && (
          ...
          {Array.from({ length: 18 }).map((_, i) => (
            <span
              ...
              style={{
                width: 6 + (i % 3) * 2,
                height: 8 + (i % 4) * 2,
                background: i % 2 === 0 ? "#B8954A" : "#D4AF72",
      ...
      <div className="relative z-10 text-center space-y-3">
        <p className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.22em] text-[#D4AF72]">
      ...
      <div className="relative z-10 mx-auto mt-8 flex justify-center">
        ...
        >
          ...
          <div
            className={cn(
              "absolute inset-x-2 top-2 h-[55%] origin-top rounded-t-lg bg-[#2D1020] border border-[#B8954A]/50 transition-transform duration-700",
          ...
          <div className="absolute inset-x-4 bottom-4 top-[42%] rounded-md bg-white/90 p-2 space-y-1.5">
            <div className="h-1.5 rounded bg-[#2D1020]/30 w-full" />
            <div className="h-1.5 rounded bg-[#2D1020]/20 w-4/5" />
      ...
      >
        {UNLOCKS.map((u) => (
          ...
          >
            <u.icon className="h-4 w-4 text-[#D4AF72] shrink-0" />
      ...
      <div className="relative z-10 mt-8 flex flex-wrap justify-center gap-3">
        <Link
          href={onContinueHref}
          className="inline-flex h-12 items-center rounded-xl bg-[#B8954A] text-[#2D1020] px-6 text-sm font-bold"
        >
