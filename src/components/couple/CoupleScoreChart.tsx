  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-baseline justify-between gap-2">
        <span className="text-sm font-semibold text-[#A07070]">{name}</span>
        <span className="font-serif text-xl font-bold text-[#A07070]">{value}%</span>
      </div>
      <div className="h-3.5 overflow-hidden rounded-full bg-[#A07070]/08">
        <div
    ...
    >
      <figcaption className="mb-4 flex flex-wrap items-end justify-between gap-2">
        <div>
          ...
          <p className="font-serif text-lg sm:text-xl font-bold text-[#A07070]">{label}</p>
        </div>
        <p className="text-sm text-[#A07070]/70">
          Convergence{" "}
          <span className="font-serif text-lg font-bold text-[#A07070]">
            {convergence}%
          </span>
        </p>
      </figcaption>
      <div className="space-y-4">
        <Bar value={scoreA} name={nameA} color="#A07070" />
        <Bar value={scoreB} name={nameB} color="#B8954A" />
      </div>
      <svg
        viewBox="0 0 200 28"
        className="mt-5 w-full text-[#A07070]/15"
        aria-hidden
      >
        <line x1="0" y1="14" x2="200" y2="14" stroke="currentColor" strokeWidth="1" />
        <circle cx={(scoreA / 100) * 200} cy="14" r="5" fill="#A07070" />
